class EncryptDecrypt {
  ...
  validateInputs() {
    ...
    if (key.length < 3) {
      this.showStatus(
        "Encryption key should be at least 3 characters long",
        "error",
      );
      this.encryptionKey.focus();
      return false;
    ...
    try {
      ...
      setTimeout(() => {
        try {
          ...
          this.showStatus(
            "❌ Decryption failed. Please check your encrypted text and key.",
            "error",
          );
        } finally {
  ...
  toggleKeyVisibility() {
    ...
    this.toggleKey.innerHTML = isPassword
  ...
  calculateKeyStrength(key) {
    ...
    if (score >= 80)
      return { percentage: 100, color: "#10b981", text: "Very Strong" };
    if (score >= 60)
      return { percentage: 80, color: "#059669", text: "Strong" };
    if (score >= 40)
      return { percentage: 60, color: "#f59e0b", text: "Medium" };
