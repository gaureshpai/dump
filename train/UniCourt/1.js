...
function solve(inputGrid) {
  ...
  const getPathAndDistance = (start, end) => {
    ...
    // Attempt 1: H-V
    let path1 = [start];
    for (let c = start.c + dc; c !== end.c + dc; c += dc)
      path1.push(Point(start.r, c));
    for (let r = start.r + dr; r !== end.r + dr; r += dr)
      path1.push(Point(r, end.c));
    if (!path1.some((p) => inputGrid[p.r][p.c] === 5)) pathAttempts.push(path1);

    // Attempt 2: V-H
    let path2 = [start];
    for (let r = start.r + dr; r !== end.r + dr; r += dr)
      path2.push(Point(r, start.c));
    for (let c = start.c + dc; c !== end.c + dc; c += dc)
      path2.push(Point(end.r, c));
    ...
    while (q.length > 0) {
      ...
      for (const [dr, dc] of dirs) {
        ...
        if (
          ...
          next.c >= 0 &&
          next.c < width &&
          inputGrid[next.r][next.c] !== 5 &&
          !visitedOnPath.has(next.id)
        ) {
  ...
  for (let i = 0; i < n; i++) {
    for (let j = i; j < n; j++) {
      ...
      const { distance, path } = getPathAndDistance(
        buildings[i].center,
        buildings[j].center,
      );
  ...
  while (tour.length < n) {
    ...
    for (let j = 0; j < n; j++) {
      if (
        !visitedTour[j] &&
        !blacklist[currIdx].has(j) &&
        dist[currIdx][j] !== Infinity
      ) {
        candidates.push(j);
      }
    }
