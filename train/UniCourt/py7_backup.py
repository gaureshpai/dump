...
def solve(input_grid):
    ...
    for i in range(len(path_buildings) - 1):
        ...
        # Create a set of cells adjacent to the destination building b2
        adjacent_to_b2 = set()
        for r in range(b2['min_r'] - 1, b2['max_r'] + 2):
            for c in range(b2['min_c'] - 1, b2['max_c'] + 2):
                if not (b2['min_r'] <= r <= b2['max_r'] and b2['min_c'] <= c <= b2['max_c']):
                    adjacent_to_b2.add((r, c))
